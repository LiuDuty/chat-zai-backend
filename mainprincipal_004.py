...
# ------------------------------------------------------------
# 2️⃣ Banco de dados (conversas) - MANTIDO PARA COMPATIBILIDADE
# ------------------------------------------------------------
def init_db():
...
def salvar_mensagem(session_id, role, content):
    ...
    # Função mantida para compatibilidade, mas não será chamada
    pass
...
async def make_api_request_with_retry(messages: list, max_retries=3) -> Tuple[bool, str]:
    ...
    # Validação das mensagens antes de enviar
    if not messages or not isinstance(messages, list):
        print("❌ Erro: messages não é uma lista válida")
        return False, "Erro interno: formato de mensagens inválido"
    
    # Valida cada mensagem
    for i, msg in enumerate(messages):
        if not isinstance(msg, dict) or 'role' not in msg or 'content' not in msg:
            print(f"❌ Erro: mensagem {i} inválida: {msg}")
            return False, f"Erro interno: mensagem {i} com formato inválido"
        
        if msg['role'] not in ['system', 'user', 'assistant']:
            print(f"❌ Erro: role inválido na mensagem {i}: {msg['role']}")
            return False, f"Erro interno: role inválido na mensagem {i}"
...
async def atualizar_e_gerar_resposta(session_id: str, nova_mensagem: str, client_ip: str = None):
    try:
        ...
        if filtro_json:
            ...
            # --- ETAPA 2.1: ENVIAR MENSAGEM COM PARÂMETROS, QUERY E RESULTADOS BRUTOS ---
            mensagem_info_busca = f"""🔍 **BUSCA REALIZADA**

📋 **Parâmetros Selecionados:**
{json.dumps(filtro_json, indent=2)}

🗄️ **Query SQL Executada:**
{sql_query}

📊 **Parâmetros da Query:**
{sql_params}

📈 **Resultados Brutos Encontrados ({len(resultados_encontrados)} imóveis):**
{resultados_encontrados}

⏳ **Agora vou formatar esses resultados para você...**"""

            # Salva a mensagem informativa no histórico
            salvar_mensagem(session_id, "system", mensagem_info_busca)
            
            # Monta o prompt com os resultados da busca
            prompt_geracao = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": nova_mensagem},
                {"role": "system", "content": mensagem_info_busca},
                {"role": "system", "content": f"Com base na busca acima, apresente esses resultados de forma clara e amigável para o usuário, sempre utilize o link https://www.openhouses.net.br/imovel/codigo_url ou use https://www.openhouses.net.br/imovel/?ref=codigo_interno . Se a lista de resultados estiver vazia, informe que nenhum imóvel foi encontrado com os critérios e sugira que ele ajuste a busca ou que pesquise diretamente no site da imobiliária https://www.openhouses.net.br/."}
            ]
            ...
            # Se o filtro estiver vazio, é uma conversa normal.
            prompt_conversa = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": nova_mensagem}
            ]
        ...
        return resposta, filtro_json if filtro_json else {}, sql_query, sql_params, resultados_encontrados
...
async def mensagem(request: Request):
    ...
    resposta, filtro_json, sql_query, sql_params, resultados_encontrados = await atualizar_e_gerar_resposta(session_id, texto, client_ip)
...
async def get_historico(session_id: str):
    # Retorna histórico vazio, pois não estamos mais salvando conversas
    return {"historico": [], "mensagem": "Funcionalidade de histórico desativada"}
